...
const HeroWithCategory: React.FC<Props> = ({
	className = "mb-12 md:mb-14 xl:mb-16", banners
}) => {
  ...
  if (!loading && isEmpty(categories?.pages?.[0].data)) {
    return (
			...
			{error ? (
				...
				<div>
					<Carousel breakpoints={categoryResponsive} buttonSize="small">
						{loading && !categories?.pages[0]?.data?.length
							...
							: categories?.pages[0]?.data?.map((category) => (
